...
impl Reader {
        ...
        RB: std::ops::RangeBounds<&'k str>,
    {
        ...
        if let Some(d) = segment.as_ref() {
            ...
            let mut decoder =
                lz4::Decoder::new(std::io::Cursor::new(d.payload)).expect("lz4 decoding");
            decoder.read_to_end(&mut data).expect("lz4 decoding 2");
        }
    ...
    RB: std::ops::RangeBounds<&'k str>,
{
    fn next_segment(&mut self) {
        ...
        if let Some(s) = self.segment.as_ref() {
            ...
            old_vec.clear();
            let mut decoder =
                lz4::Decoder::new(std::io::Cursor::new(s.payload)).expect("lz4 decoding");
            decoder.read_to_end(&mut old_vec).expect("lz4 decoding 2");
    ...
    fn next_key(&mut self) -> bool {
        while self.segment.is_some() {
            while self.pos != self.decoded.len() {
                ...
                let _fmt = std::str::from_utf8(&fmt).expect("input data is not utf8");
